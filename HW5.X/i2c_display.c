...
void write_string(char *msg, int startRow, int startCol) {
    ...
    while (msg[i]){
        ...
        //check to see if its a valid position
        if(col > 123){
            col = 0;
            row += 8;
        }
        
        //this part prints each character
        for(ii = 0; ii < 5; ii++){ 
            char charLoc = ASCII[location][ii];  //each character has 5 chars stored in this position that represent the ii column
            for(j = 7; j >=0; j--){             //each column is made up of 8 bits from top to bottom
                display_pixel_set(row-(7-j),col+ii, charLoc>>j & 0x01); 
                //colPos>>j right shifts the bit, so we work from top to bottom
                //col+ii moves it since we have an 8x5 grid
            }
        }
        
        //to move to the next position, we want to move over 5 columns
        col += 5;
